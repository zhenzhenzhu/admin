...
## 8. 收集表单数据和表单的前台验证
    1). form对象
        如何让包含<Form>的组件得到form对象?  WarpLogin = Form.create()(Login);
        WarpLogin是LoginForm的父组件, 它给LoginForm传入form属性
        用到了高阶函数和高阶组件的技术
    2). 操作表单数据
    ...
    3). 前台表单验证
        ...
        b. 自定义表单验证
            ...
            validatePwd = (rule, value, callback) => {
              if(根据条件进行判断){callback('错误提示信息')} else {callack()}
            } 
        c. 点击提示时统一验证
...
## 3. 实现登陆(维持登陆与自动登陆)
    ...
    login.jsx
        ...
        2). 如果失败, 显示错误提示信息
        3). 如果成功了: 
            维持登陆：utils中建一个local文件，使用store做兼容所有浏览器的local storage包装器
            保存user到local/内存中  
            跳转到admin
        4). 自动登陆 
            获取到内存中的user 判断
            如果内存中的user有值, 说明登陆过，自动跳转到admin
    src/index.js
        读取local中user到内存中保存
    admin.jsx
...
## 5. LeftNav组件
    1). 使用antd的组件 左侧边栏渲染 Menu / Item / SubMenu
        a:首先把需要的侧边栏数据提取到一个文件--导航菜单配置: config/menuConfig.js
        b:在LeftNav组件引入menuConfig文件进行遍历渲染 {{this.getMenuNodes(menuList)}}
        c:第一种：遍历menuList => map() 和 递归调用
        d:第二种：遍历menuList => reduce() 和 递归调用 reduce((pre,item)=>{},[初始值])
...
## 1. Header组件
    1). 界面静态布局
        三角形效果 border-top/left/right/bottom/ 显示top，其他隐藏(transparent)
    2). 获取登陆用户的名称显示(从内存中获取)
        引入 MemoryUtils 文件
        const username = memoryUtils.user.username
        渲染 {username}
    3). 当前时间
        格式化指定时间: dateUtils
        引入dateUtils文件=> import { formateDate } from '../../utils/dateUtils'
        循环定时器, 每隔1s更新当前时间状态 => componentDidMount(){} 钩子里面执行异步
        启动循环定时器，获取当前时间，更新时间
    4). 天气预报
        使用jsonp库发jsonp请求百度天气预报接口
        引入接口文件 => import {reqWeather} from '../../api/index'
        在 componentDidMount(){} 钩子里面执行异步 发送请求
        发送请求获取数据,需要配合async/await 
        => const { dayPictureUrl, weather } =await reqWeather('北京')
        更新数据 => this.setState({ dayPictureUrl, weather })
        对jsonp请求的理解 --- 见下面
    5). 当前导航项的标题
        得到当前请求的路由path: withRouter()包装非路由组件
        根据path在menuList中遍历查找对应的item的title
        首先foreach遍历 item  =》item.key === path=》说明item的title就是需要展示的title
        从 item.children中查找 cItem有值,说明匹配 => title = cItem.title
    6). 退出登陆
        Modal组件显示提示
        清除保存的user
        跳转到login
...
## 1. 添加分类
    ...
    2). 功能
        父组(Category)件得到子组件(AddForm)的数据(form) =>函数传递 setForm
        调用添加分类的接口
        重新获取分类列表
